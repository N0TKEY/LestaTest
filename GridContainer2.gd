...
# Перед этим я сделал на php+html, т.к. у нас сейчас это в ВУЗе, поэтому я понимаю, что схема может быть далеко не лучшей для Godot.
# Но с другой стороны, масштабы позволяют. 
# А для прописания проверки 8 ячеек по кругу понадобилось бы больше кода, насколько я помню по работе с шахматами.
